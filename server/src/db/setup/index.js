...
const generateFakerData = async () => {
	...
	for (let i = 0; i < 20; i++) {
		...
		const picturePool = fs.readdirSync(
			path.join(__dirname, './samplePictures'),
		);
		let pictures = [];
		for (let i = 0; i <= picCount; i++) {
			let pic = getRandomArrayIndex(picturePool);
			console.log(pic);
			await uploadFileToGcs(pic, u_id)
				.then((publicUrl) => {
					loadImg(publicUrl)
						.then((hash) => {
		...
		if (data.gender === data.preference) count++;
		await query(
			`
			INSERT INTO users (
...
const setup = async () => {
	...
	await query(`
		CREATE TABLE IF NOT EXISTS password_reset (
			...
			u_id TEXT REFERENCES users(u_id) ON DELETE CASCADE,
			hash TEXT NOT NULL,
			expires_by TIMESTAMPTZ NOT NULL
		)
	`);
