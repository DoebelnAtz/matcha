...
const DropDownComponent = ({
    ...
    return options.map(({ option, id }, index) => {
      return (
        <Option
          ...
          height={height}
          highlighted={
            state === option || (withFilter && index === selectedIndex)
          }
          onClick={(e) => {
            e.stopPropagation();
            setSelect({ option, id });
    ...
    onFilterChange
      ...
      : setOptions(
          optionList.filter(({ option }) => {
            return option.toLowerCase().includes(target.value.toLowerCase());
          })
        );
  };
  ...
  return (
    <DropDown expanded={expanded} ref={inside} width={width} height={height}>
      <CurrentOption
        expanded={expanded}
